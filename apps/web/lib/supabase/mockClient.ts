...
const applyFilters = (rows: Array<Record<string, unknown>>, filters: Array<{ field: string; op: 'eq' | 'is' | 'in'; value: unknown }>) => {
...
const attachRelations = (table: string, rows: Array<Record<string, unknown>>, select?: string) => {
  ...
  return rows.map((order) => {
    const next = { ...order };
    if (select.includes('chefs')) {
      const chef = db.chefs.find((c) => c.id === order.chef_id);
      next.chefs = chef ? { display_name: chef.display_name, slug: chef.slug } : null;
    }
    ...
    if (select.includes('customer:profiles')) {
      ...
      next.customer = customer ? { full_name: customer.full_name, phone: customer.phone } : null;
    }
    return next;
  });
};
...
const createQuery = (table: keyof MockDb) => {
  const filters: Array<{ field: string; op: 'eq' | 'is' | 'in'; value: unknown }> = [];
